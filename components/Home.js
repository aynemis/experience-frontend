...
function Home(props) {
  ...
  const navigateToSearch = (searchTerm) => {
    window.location.href = `https://experience-beta.vercel.app/search?search=${encodeURIComponent(
      searchTerm
    )}`;
  };
  ...
  useEffect(() => {
    fetch("https://experience-backend-five.vercel.app/coaches/bestCoaches")
          ...
          const coachesWithReviews = await Promise.all(
            ...
            data.coaches.map(async (coach) => {
              ...
              const reviewResponse = await fetch(
                `https://experience-backend-five.vercel.app/reviews/${coach.user.username}`
              );
